...
namespace TerminologyLauncher.FileRepositorySystem
{
    public class FileRepository
    {
        ...
        public FileRepository(string configPath)
        {
            ...
            this.CacheDirectoryInfo =
                new DirectoryInfo(Path.Combine(this.CacheRootDirectoryInfo.FullName,
                    EncodeUtils.CalculateStringMd5(this.Config.GetConfigString("fileRepositoryUrl"))));
            ...
            try
            {
                ...
                var repo =
                    JsonConverter.Parse<FileRepositoryEntity>(
                        File.ReadAllText(this.Config.GetConfigString("repoFilePath")));
            ...
            catch (WebException)
            {
                TerminologyLogger.GetLogger()
                    .Error("Unable to receive repo right now. Trying to using local repo list.");
